...
export default function SearchLocation() {
  ...
  const [recordSearchLocation, setRecordSearchLocation] = useState(false);

  //ACTION WHEN ENTER SCREENS
  const onLoad = async()=>{
        let loc = await GPS.getSearchLocation();
        if (loc) {
  ...
  return (
    <View style={{ flex: 1 }}>      
      ...
      <Text>{location ? new Date(location.timestamp).toString() : "-"}</Text>  
            ...
            <View style={{ flex: 1 }}>
                <MyMapView searchlocation={searchlocation} setSearchLocation={setSearchLocation} />
            </View>         
    </View>
  );
}
